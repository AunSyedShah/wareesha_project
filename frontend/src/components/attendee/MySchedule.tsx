...
import { Calendar, Clock, MapPin, Users, Star, Trash2, Bell, Filter, Search } from 'lucide-react';
import { useData } from '../../contexts/DataContext';
import { useAuth } from '../../contexts/AuthContext';
import { Schedule } from '../../types';

export function MySchedule() {
  const { schedules, expos, registrations } = useData();
  const { user } = useAuth();
  const [selectedDate, setSelectedDate] = useState<string>('');
  const [selectedCategory, setSelectedCategory] = useState<string>('');
  const [searchTerm, setSearchTerm] = useState('');
  const [bookmarkedSessions, setBookmarkedSessions] = useState<string[]>(['schedule-1']);
  const [registeredSessions, setRegisteredSessions] = useState<string[]>(['schedule-1']);

  const myRegistrations = registrations.filter(r => r.userId === user?.id);
  const myExpoIds = myRegistrations.map(r => r.expoId);
  const mySchedules = schedules.filter(s => myExpoIds.includes(s.expoId));
  ...
  const categories = [...new Set(mySchedules.map(s => s.category))];
  const dates = [...new Set(mySchedules.map(s => new Date(s.startTime).toDateString()))].sort();

  return (
    <div className="p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">My Schedule</h1>
        <p className="text-gray-600">Manage your registered sessions and bookmarked events</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="flex gap-2">
            <button className="flex-1 bg-fuchsia-100 text-fuchsia-700 px-4 py-2 rounded-lg hover:bg-fuchsia-200 transition-colors flex items-center justify-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-center justify-between">
            ...
            <div className="w-12 h-12 bg-fuchsia-50 rounded-lg flex items-center justify-center">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-blue-600">
                {mySchedules.filter(s => new Date(s.startTime) > new Date()).length}
              </p>
            </div>
      ...
      <div className="space-y-6">
        {Object.keys(schedulesByDate).length === 0 ? (
          <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
            ...
            <p className="text-gray-600">Register for events to see your schedule here.</p>
          </div>
        ) : (
            ...
            <div key={date} className="bg-white rounded-xl border border-gray-200 p-6">
              ...
              <div className="space-y-4">
                {schedulesByDate[date].map((session) => {
                  ...
                  const expo = expos.find(e => e.id === session.expoId);
                  
                  return (
                    <div key={session.id} className="border border-gray-200 rounded-lg p-4 hover:border-fuchsia-300 transition-colors">
